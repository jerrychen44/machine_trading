...
def moving_average_strategy(stocks_df):
    ...
    temp=stocks_df.ix[-1, 'Direction']
    stocks_df.ix[-1, 'Direction']=0.0

    for index, row in stocks_df.iterrows():
        #如果方向和之前不同, 動作, 可能是平倉或是進場
        if row["Direction"] != previouse_direction:
            if row["Direction"]== -1 and holdpostion==0 :
                stocks_df.loc[index, 'Action']='Sell'
                holdpostion=-1
            elif row["Direction"]== 0 and holdpostion != 0:
                stocks_df.loc[index, 'Action']='Close'
                holdpostion=0
            elif row["Direction"]== 1 and holdpostion==0 :
    ...
    #stocks_df["Action"] = np.sign(stocks_df["Direction"] - stocks_df["Direction"].shift(1))
    ...
    stock_signals = pd.concat([
            pd.DataFrame({"Price": stocks_df.loc[stocks_df["Signal"] == 1.0, "Close"],
                         "Regime": stocks_df.loc[stocks_df["Signal"] == 1.0, "Regime"],
                         "Signal": "Buy"}),
            pd.DataFrame({"Price": stocks_df.loc[stocks_df["Signal"] == -1.0, "Close"],
                         "Regime": stocks_df.loc[stocks_df["Signal"] == -1.0, "Regime"],
                         "Signal": "Sell"}),
