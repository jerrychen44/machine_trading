# source is from https://ntguardian.wordpress.com/2016/09/26/introduction-stock-market-data-python-2/
import pandas as pd
...
def moving_average_strategy(stocks_df):
    ...
    # We will refer to the sign of this difference as the regime; that is, if the fast moving average is above the slow moving average,
    # this is a bullish regime (the bulls rule), and a bearish regime (the bears rule) holds when the fast moving average is below the slow moving average.
    # I identify regimes with the following code.

    # np.where() is a vectorized if-else function, where a condition is checked for each component of
    # a vector, and the first argument passed is used when the condition holds, and the other passed if it does not

    stocks_df["Regime"] = np.where(stocks_df['20d-50d'] > 0, 1, 0) # if condition is true, set to 1, esle set to 0
    print(stocks_df.tail(100))
    '''
                     Open   High   Low  Close    Volume  Adj Close   20d    50d   200d  20d-50d  Regime
    Date
    2016-07-13   9.46   9.46  9.46   9.46     53000    9.02230  9.72  10.03  11.01    -0.31       0
    ...
    # and to maintain the rest of the vector, the second argument is stocks_df["Regime"], which means = 0
    stocks_df["Regime"] = np.where(stocks_df['20d-50d'] < 0, -1, stocks_df["Regime"])
    print(stocks_df.tail(100))
    '''
                 Open   High   Low  Close    Volume  Adj Close   20d    50d   200d  20d-50d  Regime
    Date
    2016-07-13   9.46   9.46  9.46   9.46     53000    9.02230  9.72  10.03  11.01    -0.31      -1
    ...
    # Regime -1 is fastavg under slowavg line, +1 is fastaveline above slowavgline
    '''
    stocks_df.loc['2016-01-01':'2016-11-29',"Regime"].plot(ylim = (-2,2)).axhline(y = 0, color = "black", lw = 2)
    plt.show()
    stocks_df["Regime"].plot(ylim = (-2,2)).axhline(y = 0, color = "black", lw = 2)
    plt.show()
    '''

    print(stocks_df["Regime"].value_counts())
    '''
    -1    897 => 897 days are the fastavg line under slowavg line
     1    710 => 710 days are fastavg line above slowavg line
     0     72 => to see as neutral. (fastavgline = slowavgline)
    ...
    # ==> the trading signal appears at the regime changes. -1 => 1,0 or 1 => -1,0
    # ==> Regime means you should have the long(1) or short(-1) position or have no position(0).
    ###########################
    # To ensure that all trades close out, I temporarily change the regime of the last row to 0
    regime_orig = stocks_df.ix[-1, "Regime"]
    stocks_df.ix[-1, "Regime"] = 0 #because we force the latest day regime to 0, whatever holding postion before that day, it will close.

    # Signal_t=sign(Regime_t-Regime_t-1), and Signal_t will only be -1,0,1
    # with -1 indicating “sell”, 1 indicating “buy”, and 0 no action

    #The sign function returns -1 if x < 0, 0 if x==0, 1 if x > 0
    # if regime has the same direction in t day and t-1 day, then Do not anything. ex: 1-1 =0, -1-(-1)=0
    # if regime has different direction happened, which means the signal appears.
    # ex: 1-(-1)=2, sign(2)=1 (close original sell positon), ex: -1-(1)=-2,sign(-2)=-1 (close original buy positon)
    # ex: 1-(0)=1, sign(1)=1, (buy new position), ex:-1-(0)=-1, (short new position)
    # ex: 0-(1)=-1 sign(-1)=-1 (close original buy positon), ex: 0-(-1)=1, (close original sell positon)
    stocks_df["Signal"] = np.sign(stocks_df["Regime"] - stocks_df["Regime"].shift(1))
    # Restore original regime data
    stocks_df.ix[-1, "Regime"] = regime_orig
    print(stocks_df.tail(100))
    '''
                 Open   High   Low  Close    Volume  Adj Close   20d    50d   200d  20d-50d  Regime  Signal
    Date
    2016-07-13   9.46   9.46  9.46   9.46     53000    9.02230  9.72  10.03  11.01    -0.31    -1.0     0.0
    2016-08-22   9.30   9.31  9.17   9.17  12181000    9.17000  9.47   9.65  10.72    -0.18    -1.0     0.0
    2016-08-23   9.19   9.27  9.19   9.20   8157000    9.20000  9.46   9.63  10.71    -0.17    -1.0     0.0
    2016-08-24   9.24   9.24  9.17   9.20   6887000    9.20000  9.45   9.61  10.70    -0.16    -1.0     0.0
    2016-08-25   9.20   9.27  9.18   9.25   9246000    9.25000  9.43   9.59  10.69    -0.16    -1.0     0.0
    2016-08-26   9.27   9.29  9.22   9.28   6416000    9.28000  9.42   9.58  10.68    -0.16    -1.0     0.0
    2016-08-29   9.27   9.29  9.21   9.27   7648000    9.27000  9.41   9.56  10.67    -0.15    -1.0     0.0
    2016-08-30   9.29   9.30  9.27   9.28   6049000    9.28000  9.40   9.54  10.66    -0.14    -1.0     0.0
    2016-08-31   9.29   9.29  9.21   9.22  11057000    9.22000  9.38   9.53  10.65    -0.15    -1.0     0.0
    2016-09-01   9.21   9.26  9.19   9.25   5725000    9.25000  9.36   9.51  10.64    -0.15    -1.0     0.0
    2016-09-02   9.38   9.49  9.38   9.42  16208000    9.42000  9.36   9.50  10.63    -0.14    -1.0     0.0
    2016-09-05   9.44   9.48  9.40   9.42   6019000    9.42000  9.36   9.49  10.63    -0.13    -1.0     0.0
    2016-09-06   9.42   9.49  9.40   9.47   8723000    9.47000  9.35   9.48  10.62    -0.13    -1.0     0.0
    2016-09-07   9.50   9.57  9.50   9.56   9929000    9.56000  9.36   9.48  10.61    -0.12    -1.0     0.0
    2016-09-08   9.56   9.64  9.51   9.63  11909000    9.63000  9.37   9.48  10.61    -0.11    -1.0     0.0
    2016-09-09   9.60   9.60  9.50   9.56   6677000    9.56000  9.37   9.48  10.60    -0.11    -1.0     0.0
    2016-09-12   9.43   9.43  9.35   9.38  10013000    9.38000  9.36   9.47  10.59    -0.11    -1.0     0.0
    2016-09-13   9.38   9.39  9.25   9.30   6467000    9.30000  9.35   9.47  10.58    -0.12    -1.0     0.0
    2016-09-14   9.28   9.36  9.27   9.30   5910000    9.30000  9.35   9.46  10.58    -0.11    -1.0     0.0
    2016-09-15   9.30   9.30  9.30   9.30         0    9.30000  9.34   9.47  10.57    -0.13    -1.0     0.0
    2016-09-16   9.30   9.30  9.30   9.30         0    9.30000  9.34   9.47  10.56    -0.13    -1.0     0.0
    2016-09-19   9.28   9.40  9.28   9.36   6202000    9.36000  9.35   9.47  10.55    -0.12    -1.0     0.0
    2016-09-20   9.37   9.47  9.37   9.45   6412000    9.45000  9.36   9.47  10.54    -0.11    -1.0     0.0
    2016-09-21   9.45   9.49  9.38   9.48   4602000    9.48000  9.37   9.47  10.54    -0.10    -1.0     0.0
    2016-09-22   9.50   9.51  9.39   9.46   5092000    9.46000  9.38   9.47  10.53    -0.09    -1.0     0.0
    2016-09-23   9.44   9.57  9.44   9.52  10602000    9.52000  9.40   9.47  10.52    -0.07    -1.0     0.0
    2016-09-26   9.56   9.56  9.44   9.44   5964000    9.44000  9.40   9.46  10.51    -0.06    -1.0     0.0
    2016-09-27   9.44   9.44  9.44   9.44         0    9.44000  9.41   9.45  10.50    -0.04    -1.0     0.0
    2016-11-08   9.40   9.40  9.33   9.38   5153000    9.38000  9.41   9.42  10.19    -0.01    -1.0     0.0
    2016-11-09   9.40   9.42  9.16   9.16  11273000    9.16000  9.40   9.42  10.18    -0.02    -1.0     0.0
    2016-11-10   9.26   9.33  9.26   9.28   4910000    9.28000  9.39   9.42  10.17    -0.03    -1.0     0.0
    2016-11-11   9.25   9.25  9.15   9.19  11161000    9.19000  9.38   9.41  10.16    -0.03    -1.0     0.0
    2016-11-14   9.20   9.25  9.17   9.25   5871000    9.25000  9.38   9.41  10.15    -0.03    -1.0     0.0
    2016-11-15   9.23   9.24  9.17   9.19   6363000    9.19000  9.37   9.40  10.14    -0.03    -1.0     0.0
    2016-11-16   9.19   9.25  9.17   9.17   6313000    9.17000  9.36   9.40  10.13    -0.04    -1.0     0.0
    2016-11-17   9.21   9.25  9.17   9.20   7218000    9.20000  9.35   9.39  10.12    -0.04    -1.0     0.0
    2016-11-18   9.18   9.24  9.18   9.24   4587000    9.24000  9.34   9.38  10.11    -0.04    -1.0     0.0
    2016-11-21   9.25   9.27  9.23   9.27   4651000    9.27000  9.33   9.38  10.11    -0.05    -1.0     0.0
    2016-11-22   9.40   9.49  9.34   9.35  16130000    9.35000  9.33   9.38  10.10    -0.05    -1.0     0.0
    2016-11-23   9.41   9.48  9.41   9.46  11762000    9.46000  9.33   9.38  10.09    -0.05    -1.0     0.0
    2016-11-24   9.60  10.30  9.60  10.25  86535000   10.25000  9.38   9.40  10.08    -0.02    -1.0     0.0
    2016-11-25  10.10  10.10  9.77   9.77  40020000    9.77000  9.40   9.41  10.07    -0.01    -1.0     0.0
    2016-11-28   9.78   9.84  9.66   9.68  13759000    9.68000  9.40   9.42  10.06    -0.02    -1.0     0.0
    2016-11-29   9.71   9.76  9.55   9.60  13723000    9.60000  9.40   9.42  10.05    -0.02    -1.0     1.0
    '''

    stocks_df["Signal"].plot(ylim = (-2, 2))
    plt.show()
    print(stocks_df["Signal"].value_counts())
    '''
     0.0    1617 => days do nothing. (keep original postion (including no position))
     1.0      31 => 31 times do the buy action.(may first long position or close original short position)
    -1.0      30
    Name: Signal, dtype: int64
    '''

    #Print out the Date and price that every bugy and every sell
    print(stocks_df.loc[stocks_df["Signal"] == 1, "Close"],len(stocks_df.loc[stocks_df["Signal"] == 1, "Close"]))
    '''
        Date
    2010-08-09    19.2884
    2010-12-15    23.9869
    ...
    2016-11-04     9.3500
    2016-11-07     9.3900
    Name: Close, dtype: float64,30
    '''

    # Create a DataFrame with trades, including the price at the trade and the regime under which the trade is made.
    stock_signals = pd.concat([
            pd.DataFrame({"Price": stocks_df.loc[stocks_df["Signal"] == 1, "Close"],
                         "Regime": stocks_df.loc[stocks_df["Signal"] == 1, "Regime"],
                         "Signal": "Buy"}),
            pd.DataFrame({"Price": stocks_df.loc[stocks_df["Signal"] == -1, "Close"],
                         "Regime": stocks_df.loc[stocks_df["Signal"] == -1, "Regime"],
                         "Signal": "Sell"}),
    ...
    2010-08-09  19.2884     1.0    Buy
    2010-11-26  22.4537    -1.0   Sell
    2010-12-15  23.9869     1.0    Buy
    2011-01-25  22.2558    -1.0   Sell
    2011-04-28  18.2498     1.0    Buy
    2011-06-16  17.8541    -1.0   Sell
    2011-06-27  19.5357     1.0    Buy
    2011-06-28  19.2884     0.0   Sell
    2011-07-05  19.2390     1.0    Buy
    2011-07-25  17.3596    -1.0   Sell
    2011-10-26  14.8867     1.0    Buy
    2011-11-23  12.3149    -1.0   Sell
    2012-01-16  14.3000     1.0    Buy
    2012-03-09  13.8500    -1.0   Sell
    2012-05-28  11.3500     1.0    Buy
    2012-08-03  12.5500    -1.0   Sell
    2012-10-08  12.2000     0.0    Buy
    2012-10-09  12.2500     1.0    Buy
    2012-11-01  11.4500    -1.0   Sell
    2012-12-14  12.3000     1.0    Buy
    2013-02-26  12.2000     0.0   Sell
    2013-02-27  12.2000    -1.0   Sell
    2013-05-27  11.9000     0.0    Buy
    2013-05-28  12.0000     1.0    Buy
    2013-06-26  11.2000     0.0   Sell
    2013-06-27  11.2000    -1.0   Sell
    2013-10-02  11.1000     1.0    Buy
    2013-10-29  10.6000    -1.0   Sell
    2013-12-09  10.6500     0.0    Buy
    2013-12-10  10.7000     1.0    Buy
    2014-01-31  10.6500     0.0   Sell
    2014-02-05  10.5000    -1.0   Sell
    2014-05-01  10.1000     0.0    Buy
    2014-05-05  10.1500    -1.0   Sell
    2014-05-06  10.1500     0.0    Buy
    2014-05-07  10.2500     1.0    Buy
    2014-06-09  10.1500     0.0   Sell
    2014-06-11  10.1000    -1.0   Sell
    2014-07-18  10.3000     0.0    Buy
    2014-07-21  10.3000     1.0    Buy
    2014-08-14  10.0000    -1.0   Sell
    2014-09-25  10.2000     1.0    Buy
    2015-02-23  15.0000    -1.0   Sell
    2015-03-23  15.9500     0.0    Buy
    2015-03-24  16.2000     1.0    Buy
    2015-05-13  14.6500    -1.0   Sell
    2015-07-21  14.1500     1.0    Buy
    2015-08-24  11.3500    -1.0   Sell
    2015-10-27  12.0500     1.0    Buy
    2015-11-26  11.0000     0.0   Sell
    2015-11-27  10.9000    -1.0   Sell
    2016-01-01  12.0000     1.0    Buy
    2016-01-29  11.2500    -1.0   Sell
    2016-03-07  11.6500     1.0    Buy
    2016-04-11  11.0500    -1.0   Sell
    2016-09-29   9.5000     1.0    Buy
    2016-10-06   9.5000     0.0   Sell
    2016-10-10   9.4400     1.0    Buy
    2016-11-04   9.3500     0.0   Sell
    2016-11-07   9.3900    -1.0   Sell
    2016-11-29   9.6000    -1.0    Buy
    '''
    ...
    #################
    print(len(stock_signals.loc[(stock_signals["Signal"] == "Buy") & stock_signals["Regime"] == 1, "Price"]))
    print(len(pd.Series(stock_signals["Price"] - stock_signals["Price"].shift(1)).loc[stock_signals.loc[(stock_signals["Signal"].shift(1) == "Buy") & (stock_signals["Regime"].shift(1) == 1)].index].tolist()))
    print(len(stock_signals["Price"].loc[stock_signals.loc[(stock_signals["Signal"].shift(1) == "Buy") & (stock_signals["Regime"].shift(1) == 1)].index].index))
    # Let's see the profitability of long trades
    stock_long_profits = pd.DataFrame({
            "Price": stock_signals.loc[(stock_signals["Signal"] == "Buy") & stock_signals["Regime"] == 1, "Price"],
    ...
    pandas_candlestick_ohlc(stocks_df, stick = 45, otherseries = ["20d", "50d", "200d"])
    return 0
