...
def moving_average_strategy(stocks_df):
    ...
    2016-09-29   9.49   9.55  9.47   9.50   8310000    9.50000  9.44   9.43  10.48     0.01        1.0  SClose;Buy
    ...
    '''
    # Collect the data only has Action
    stocks_df_action = stocks_df[ (stocks_df["Action"]!= 0) ]
    print(stocks_df_action)

    ###################
    # Build the traiding Book Sheet
    #################
    # Let's see the profitability of long trades
    stock_trading_sheet = pd.DataFrame({

            "Start Date": stocks_df_action[~stocks_df_action['Action'].str.contains("Close")].index,
            "Action":     stocks_df_action[~stocks_df_action['Action'].str.contains("Close")]['Action'],
            #"Close Date":stocks_df.loc[(stocks_df["Action"] == "Buy") , "Action"]
            })
    #stock_long_profits.sort_index(inplace = True)
    print(stock_trading_sheet,stock_trading_sheet.shape[0])
